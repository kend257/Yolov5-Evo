...
def autopad(k, p=None, d=1):
    """
    # 自动计算卷积层的填充（padding）大小，以确保输出形状与输入形状相同（即 'same' 输出形状）。
    # 这个函数考虑了可选的膨胀（dilation）参数。
    k：卷积核的大小，可以是整数（表示方形卷积核）或列表（表示非方形卷积核）。
    p：填充大小，可以是整数或列表。如果为 None，则自动计算。
    d：膨胀系数，默认值为 1。
    `k`: kernel, `p`: padding, `d`: dilation.
    """
    if d > 1:
        k = d * (k - 1) + 1 if isinstance(k, int) else [d * (x - 1) + 1 for x
                                                        in k]  # 处理膨胀（dilation)
    if p is None:
